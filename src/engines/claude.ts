...
export class ClaudeEngine implements Engine {
  ...
  run(opts: EngineRunOptions): EngineRun {
    const args = buildClaudeArgs(opts, this.engineOpts);
...
/**
 * Pure builder for the `claude` CLI argv. The per-run effort (opts.effort)
 * overrides the engine-wide default (engineOpts.effort).
 */
export function buildClaudeArgs(
  opts: EngineRunOptions,
  engineOpts: ClaudeEngineOptions = {},
): string[] {
  const args = [
    '-p',
    '--verbose',
  ...
  const effort = opts.effort ?? engineOpts.effort;
  if (effort) args.push('--effort', effort);
  if (engineOpts.maxBudgetUsd !== undefined) {
    args.push('--max-budget-usd', String(engineOpts.maxBudgetUsd));
  }
  if (opts.model) args.push('--model', opts.model);
  if (opts.resumeSessionId) args.push('--resume', opts.resumeSessionId);
  // Branch a resumed conversation into a fresh session id (only with --resume).
  if (opts.forkSession && opts.resumeSessionId) args.push('--fork-session');
  return args;
}
...
export async function* mapClaudeEvents(src: JsonlSource): AsyncGenerator<AgentEvent, void, void> {
        ...
        yield {
          ...
          text:
            typeof ev.result === 'string'
              ...
              : ok
                ? ''
                : `Claude exited with an error (${String(ev.subtype ?? 'unknown')})`,
  ...
  if (!sawResult) {
    ...
    yield {
      kind: 'error',
      message:
        `claude exited (code ${exit.code}${sig}) without a result event` +
        (exit.stderrTail ? `\n${exit.stderrTail}` : ''),
    };
  }
}
