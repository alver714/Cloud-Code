...
export function formatUsage(usage: Usage | undefined): string | undefined {
  ...
  // Показываем только сгенерированные токены: input раздут перечитыванием
  // кэша контекста на каждом шаге агента и выглядит неправдоподобно.
  const out = usage.outputTokens ?? 0;
  if (out > 0) return `${formatTok(out)} tok ответа`;
