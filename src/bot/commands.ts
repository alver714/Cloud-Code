...
/create имя [private|public] [claude|codex] — новый репозиторий с нуля + сессия
/chat [claude|codex] — просто чат с агентом, без репозитория
...
/context — окно контекста выбранной модели и сколько занято
/usage — лимиты подписок Claude и Codex + расход бота за сегодня + состояние VM
/cleanup — удалить workspaces без привязанной сессии (освободить диск)
...
export async function sendRepoKeyboard(ctx: Context, title: string): Promise<void> {
  ...
  try {
    ...
    kb.text('💬 Просто чат (без репозитория)', 'chat:new').row();
...
export const CHAT_REPO_LABEL = '💬 чат';
...
export async function startChatFlow(
  ...
  try {
    ...
    await ctx.api.sendMessage(
      chatId,
      `❌ Не смог создать workspace чата:\n<pre>${truncateHtml(escapeHtml(String(err)), 800)}</pre>`,
  ...
  await ctx.api.sendMessage(
    chatId,
    [
      `💬 <b>Чат готов</b> — без репозитория`,
      `🤖 ${engine}`,
      '',
      'Просто пиши сюда. У агента есть все его инструменты (веб, Telegram-плагин, песочница для черновиков) — контекст разговора сохраняется как в обычной сессии.',
    ].join('\n'),
    { message_thread_id: topicId, parse_mode: 'HTML' },
  );
}
...
/** «🖥 Сервер» — RAM / диск / egress. Skips null collectors; [] if all null. */
function renderServerSection(rs: ResourceStatus | null): string[] {
  if (!rs) return [];
  const body: string[] = [];
  if (rs.memory) {
    const m = rs.memory;
    let l = `RAM: ${Math.round(m.availableMb)} / ${Math.round(m.totalMb)} MB свободно`;
  ...
  if (rs.disk) {
    ...
    body.push(`${mark}диск: ${rs.disk.usedPct}% занято из ${Math.round(rs.disk.totalGb)} GB`);
  }
  if (rs.egress && rs.egressUsedPct !== null) {
    ...
    body.push(
      `${mark}egress: ${Math.round(rs.egress.monthTxMb)} / ${rs.egressFreeMb} MB ` +
        `(${rs.egressUsedPct}%) · до конца месяца ${rs.daysLeftInMonth} дн.`,
    );
  }
  return body.length > 0 ? ['', '🖥 <b>Сервер</b>', ...body] : [];
}
...
export function registerCommands(bot: Bot, deps: BotDeps): void {
  ...
  bot.command('create', async (ctx) => {
    ...
    if (!isValidNewRepoName(name) || rest.some((r) => !known.has(r))) {
      await reply(
        ctx,
        'Формат: <code>/create имя [private|public] [claude|codex]</code> — новый репозиторий (по умолчанию приватный)',
      );
      return;
    }
    ...
    const note = await reply(ctx, `⏳ Создаю репозиторий <code>${escapeHtml(name)}</code>…`);
    let full: string;
    try {
    ...
    await ctx.api.editMessageText(
      ctx.chat!.id,
      note.message_id,
      `✅ Репозиторий <code>${escapeHtml(full)}</code> создан (${visibility}).`,
      { parse_mode: 'HTML' },
    );
  ...
  bot.command('chat', async (ctx) => {
    ...
    if (arg && arg !== 'claude' && arg !== 'codex') {
      await reply(ctx, 'Формат: <code>/chat [claude|codex]</code> — чат без репозитория');
      return;
    }
  ...
  const contextWindowCommand = async (ctx: Context) => {
    const s = requireSession(ctx, deps);
    if (!s) return;
    const model = s.model ?? cfg.defaultModels[s.engine] ?? 'дефолт CLI';
    const used = s.contextUsedTokens;
    const window = s.contextWindowTokens;
    const lines = [`🧠 Модель: <b>${escapeHtml(model)}</b>`];
    if (window !== undefined) {
      const usedLabel = used === undefined ? 'нет данных' : `${formatTok(used)} токенов`;
      const percent = used === undefined ? '' : ` (${((used / window) * 100).toFixed(1)}%)`;
      lines.push(`Окно контекста: <b>${formatTok(window)}</b> токенов`);
      lines.push(`Занято: <b>${usedLabel}</b>${percent}`);
    } else if (used !== undefined) {
      lines.push(`Занято: <b>${formatTok(used)}</b> токенов`);
      lines.push('Размер окна модель пока не сообщила.');
    } else {
      lines.push('Данных пока нет — отправь хотя бы один промпт в этой сессии.');
    }
    await reply(ctx, lines.join('\n'));
  };
  ...
  bot.command('cleanup', async (ctx) => {
    if (!ctx.chat) return;
    const note = await reply(ctx, '🧹 Ищу неиспользуемые workspaces…').catch(() => undefined);
    const liveWorkdirs = store.all().map((s) => s.workdir);
    const orphans = await findOrphanWorkspaces(cfg.workspacesDir, liveWorkdirs);

    if (orphans.length === 0) {
      const text = '✨ Нечего чистить — все директории в <code>workspaces</code> заняты живыми сессиями.';
      if (note) {
        await ctx.api
    ...
    const listing = orphans
      ...
      .join('\n');
    const text =
      `🧹 <b>Неиспользуемые workspaces</b> (${orphans.length})\n${listing}\n\n` +
      `Освободится ~<b>${humanBytes(totalBytes)}</b>. Это директории без привязанной сессии.`;
    const kb = new InlineKeyboard()
      .text(`🧹 Удалить ${orphans.length} дир.`, 'cleanup:go')
  ...
  bot.callbackQuery(/^cleanup:go$/, async (ctx) => {
    ...
    if (!orphans) {
      await ctx.answerCallbackQuery({ text: 'Список устарел — вызови /cleanup ещё раз.' }).catch(() => undefined);
      return;
    }
    cleanupCache.delete(key);
    await ctx.answerCallbackQuery({ text: 'Удаляю…' }).catch(() => undefined);
    const res = await deleteWorkspaces(cfg.workspacesDir, orphans);
    const refusedPart = res.refused > 0 ? ` (пропущено небезопасных путей: ${res.refused})` : '';
    await reply(
      ctx,
      `🧹 Удалено директорий: <b>${res.deleted}</b>, освобождено ~<b>${humanBytes(res.freedBytes)}</b>${refusedPart}.`,
    ).catch(() => undefined);
  });
