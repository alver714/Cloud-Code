...
if [[ -z "$GCP_PROJECT" || "$GCP_PROJECT" == "(unset)" ]]; then
  echo "GCP project is not set: export GCP_PROJECT=... or gcloud config set project ..." >&2
  exit 1
fi

# Free tier: e2-micro is free only in us-west1 / us-central1 / us-east1
...
BOOT_DISK_SIZE="${BOOT_DISK_SIZE:-30GB}"      # Always Free: up to 30GB pd-standard
